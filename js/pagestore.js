...
const PageStore = class {
    ...
    getNetFilteringSwitch() {
        // ⚠️ VNSAB: Hedef site kontrolü
        if (typeof window.VNSAB_shouldBlockDomain === 'function') {
            try {
                const hostname = this.tabHostname;
                const shouldBlock = window.VNSAB_shouldBlockDomain(hostname);
                if (!shouldBlock) {
                    // Hedef site değilse filtreleme KAPALI
                    console.log('[VNSAB-NETSWITCH] 🚫 Filtering OFF for:', hostname);
                    return false;
                }
                console.log('[VNSAB-NETSWITCH] ✅ Filtering ON for:', hostname);
            } catch (e) {
                console.error('[VNSAB-NETSWITCH] Error:', e);
            }
        }
        
        // Normal uBlock kontrolü
