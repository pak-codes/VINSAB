...
vAPI.Net = class {
    ...
    onBeforeSuspendableRequest(details) {
        // ⚠️ VNSAB: Hedef site kontrolü - EN ÖNCE BU!
        if (typeof window.VNSAB_shouldBlockDomain === 'function') {
            try {
                const url = new URL(details.url);
                const hostname = url.hostname;
                const shouldBlock = window.VNSAB_shouldBlockDomain(hostname);
                
                if (!shouldBlock) {
                    // ⚠️ KRİTİK: Hedef site değilse TAMAMEN ATLA
                    // onUnprocessedRequest'e gitme, suspendableListener'a gitme
                    // SADECE undefined dön ve isteği geçir
                    if (details.type === 'main_frame') {
                        console.log('[VNSAB-FILTER] 🚫 Hedef değil - uBlock tamamen BYPASS:', hostname);
                    }
                    return; // Hiçbir işlem yapma, isteği olduğu gibi geçir
                }
                
                if (details.type === 'main_frame') {
                    console.log('[VNSAB-FILTER] 🎯 HEDEF site - uBlock devreye giriyor:', hostname);
                }
            } catch (e) {
                // URL parse hatası olursa normal uBlock işleme devam et
                console.error('[VNSAB-FILTER] ⚠️ URL parse hatası, normal uBlock çalışacak:', e);
            }
        } else if (details.type === 'main_frame') {
            console.warn('[VNSAB-FILTER] ⚠️ VNSAB fonksiyonu bulunamadı, normal uBlock çalışıyor');
        }
        
        // Buraya sadece HEDEF siteler gelir (veya VNSAB fonksiyonu yoksa)
        if ( this.suspendableListener !== undefined ) {
            const result = this.suspendableListener(details);
            if (details.type === 'main_frame' && result && result.cancel) {
                console.log('[VNSAB-FILTER] 🔴 uBlock tarafından ENGELLENDİ:', details.url);
            }
            return result;
        }
    ...
    makeNewListenerProxy(clientListener) {
        const actualListener = details => {
            ...
            // ⚠️ VNSAB: TÜM webRequest listener'ları için hedef kontrolü!
            if (typeof window.VNSAB_shouldBlockDomain === 'function') {
                try {
                    const url = new URL(details.url);
                    const hostname = url.hostname;
                    ...
                    if (!shouldBlock) {
                        // Hedef site değilse bu listener'ı ATLA
                        if (details.type === 'main_frame') {
                            console.log('[VNSAB-PROXY] 🚫 Listener bypass (hedef değil):', hostname);
                        }
                        return; // undefined döndür, hiçbir işlem yapma
                    }
                    
                    if (details.type === 'main_frame') {
                        console.log('[VNSAB-PROXY] ✅ Listener çalışıyor (hedef site):', hostname);
                    }
                } catch (e) {
                    // URL parse hatası, normal devam et
                }
            }
